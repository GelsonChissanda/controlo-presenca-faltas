...
export default function NotificacoesDoEncarregadoScreen() {
  const router = useRouter();
  const { encarregadoId, nome } = useLocalSearchParams();
  ...
  useEffect(() => {
    ...
    const q = query(
      ...
      where("destinatario_id", "==", encarregadoId),
    ...
    const unsubscribe = onSnapshot(
      ...
      (erro) => {
        console.log("Erro ao carregar notificações do encarregado:", erro);
        setLoading(false);
      }
    );
  ...
  return (
    <ScrollView className="flex-1 bg-gray-50">
      <View className="bg-slate-900 pt-14 pb-6 px-6">
        ...
        <Text className="text-white text-2xl font-bold">{nome || "Encarregado"}</Text>
      ...
      {!loading && notificacoes.length === 0 && (
        <View className="px-6 pt-12 items-center">
          ...
          <Text className="text-gray-400 text-center">
            Este encarregado ainda não tem notificações.
          </Text>
        </View>
      )}
